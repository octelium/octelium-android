...
current="$(sed -n 's/^octelium.version=\(.*\)$/\1/p' "${PROPERTIES}")"
...
IFS=. read -r _ minor_next patch_next <<< "${version}"
...
sed -i.bak "s/^octelium.version=.*/octelium.version=${version}/" "${PROPERTIES}"
rm -f "${PROPERTIES}.bak"
